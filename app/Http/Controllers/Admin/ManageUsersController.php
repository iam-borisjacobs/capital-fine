...
class ManageUsersController extends Controller
{
    ...
    public function delsystemuser($id)
    {
        ...
        //delete IrsRefund
        $IrsRefunds = IrsRefund::where('user_id', $id)->get();
        if (!empty($IrsRefunds)) {
            foreach ($IrsRefunds as $IrsRefund) {
                IrsRefund::where('id', $IrsRefund->id)->delete();
            }
        }
       ...
       $card_transactions = CardTransaction::where('user_id', $id)->get();
        if (!empty($card_transactions)) {
            foreach ($card_transactions as $card_transaction) {
                CardTransaction::where('id', $card_transaction->id)->delete();
            }
        }
    ...
    public function useage(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'User Useage details updated Successfully!');
    }

    //useage ends


    // authorization codes start
    public function authorizationcodes(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'User Banking Authorization Codes updated Successfully!');
    }



    //authorization codes ends
